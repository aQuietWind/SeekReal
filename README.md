...
| 技术                |     版本     | 说明          | 必要性                                              |
|-------------------|:----------:|-------------|--------------------------------------------------|
| Spring Boot       |   3.5.14   | Web服务快速构建框架 | 必须（你说呢）                                          |
| MyBatis           |   3.0.5    | 数据库连接框架     | 必须（你说呢）                                          |                                     
| Caffeine          |   3.1.8    | JVM本地缓存支持   | 少量使用                                             |
| JWT               |   0.13.0   | 帐号验证        | 几乎不可消除                                             |
| Redis             |   8.6.1    | 分布式缓存支持     | 许多场景使用，且作用难以替换                                   |
| MySQL             |   8.0.46   | 数据库服务       | 必须（你说呢）                                          |  
| Elasticsearch     |   9.3.2    | 搜索引擎服务      | 必须（你说呢）                                          |
| RabbitMQ          |   3.13.7   | 消息中间件       | 几乎不可消除                                           |                               
| Nacos             |   2.0.0    | 注册中心        | 必须（你说呢）                                          |
| Fegin             | 2025.0.2 | 模块间通信       | 少量使用                                             |
| Gateway           | 2025.0.2 | 网关层核心       | 几乎不可消除                                          |
| Sentinel          |    2.4     | 流量控制工具      | 可有可无，但系统有一定依赖性<small>（指的是配置文档的配置和Config类）</small> |
